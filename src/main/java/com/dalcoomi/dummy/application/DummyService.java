...
public class DummyService {
	...
	public DummyDataResponse generateDummyData(String type, int count) {
		try {
			return switch (type.toLowerCase()) {
				...
				case "category" -> generateCategory();
				case "transaction" -> generateTransactions(count);
				default -> new DummyDataResponse(false,
					"지원하지 않는 타입입니다. member, category, transaction 중 하나를 선택하세요.", type, 0, null);
			};
		} catch (Exception e) {
	...
	private DummyDataResponse generateCategory() {
		...
		// 랜덤 카테고리명 생성 (타임스탬프 사용하여 중복 방지)
		String categoryName = "카테고리" + System.currentTimeMillis();
		...
		data.put("creatorId", member.getId());
		data.put("creatorNickname", member.getNickname());

		log.info("더미 카테고리 생성 완료 - name: {}", category.getName());

		return new DummyDataResponse(true, "카테고리 생성 완료", "category", 1, data);
	}
	...
	private DummyDataResponse generateTransactions(int count) {
		...
		List<Category> categories = categoryRepository.findMyCategories(firstMember.getId(), EXPENSE);
		...
		if (categories.isEmpty()) {
			...
			categories = categoryRepository.findMyCategories(firstMember.getId(), EXPENSE);
		}
